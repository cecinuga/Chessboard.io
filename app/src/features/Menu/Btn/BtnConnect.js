...
export default function BtnConnect({ id, names, href, img, classes, current }) {
    ...
    store.subscribe(async()=>{
      if(store.getState().menu.status=='logout'){ window.location.reload(); }
    });

    return (
        <div className={"Btn-Connect inline-block mr-2 mt-3 bg-orange-600 hover:bg-orange-500 p-1 px-2 rounded-full border-2 border-solid text-white border-orange-800 mb-2 font-semibold "+classes}>
