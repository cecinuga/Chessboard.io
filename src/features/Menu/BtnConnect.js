...
import { logHandler, useMenu } from './menuSlice';
...
export default function BtnConnect({ id, names, href, img, classes, current }) {
    ...
    useEffect(() => {
      if (isAuthenticated) {
        ...
        console.log(store.getState())
        dispatch(logHandler({isAuthenticated})).then(console.log(store.getState()));
    ...
    return (
        <div className={"Btn-Connect inline-block mr-2 mt-2 bg-red-400 p-1 text-gray-200 rounded border-2 border-solid border-red-600 mb-2"+classes}>
            <button id="btn-connect-header" className="Connect-Link" onClick={()=>{ dispatch(logHandler()).then(console.log(store.getState())); }} >{names[0]}</button>
